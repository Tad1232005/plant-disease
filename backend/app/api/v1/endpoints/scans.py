...
from app.schemas.scan import GradcamResponse, ScanDetailResponse, ScanHistoryItem
...
@router.get("/{scan_id}/gradcam", response_class=FileResponse)
def get_gradcam_image(
    scan_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> FileResponse:
    path = scan_service.get_scan_gradcam(db, current_user, scan_id)
    return FileResponse(
        path,
...
@router.post("/{scan_id}/gradcam", response_model=GradcamResponse)
def create_gradcam_image(
    scan_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> dict[str, str | int]:
    gradcam_path = scan_service.create_scan_gradcam(db, current_user, scan_id)
    return {"scan_id": scan_id, "gradcam_path": gradcam_path}
